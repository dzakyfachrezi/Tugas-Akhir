...
[signal,Fs,globaltm] = rdsamp('ARR_12');
ch_annotate = 2; % channel used to annotate
f_ch = 2;
pol_res = [0,0,0,0,0];
no_seg = 60;
%dat = 'NR-02';
...
for i = 1:length(globalmR)
    if globalmR(i,1) <= 50
        ...
        globalmQ(i,1) = tmp;
        [~,tmp] = min(globalmECG_pp(globalmR(i,1):globalmR(i,1)+50));
        globalmS(i,1) = tmp + globalmR(i,1) - 1;
    elseif globalmR(i,1) + 50 >= length(globalmECG_pp)
        [~,tmp] = min(globalmECG_pp(globalmR(i,1)-50:globalmR(i,1)));
        globalmQ(i,1) = tmp + (globalmR(i,1) - 50) - 1;
...
endPt = startPt + 10000; % ending index
globalsnr = zeros(no_seg,length(a_ch));
...
while endPt <= length(globalmECG_pp)
    ...
    ext_row = 5000; % extra row for adapting the weight
    ...
    for j = 1:length(a_ch)
        ...
        for i = 1:length(mR)
            if mR(i,1) <= 50
                residu(1:mR(i,1) + 50,j) = 0;
            elseif mR(i,1) + 50 >= length(residu(:,j))
                residu(mR(i,1) - 50:end,j) = 0;
            else
                residu(mR(i,1) - 50:mR(i,1) + 50,j) = 0;
            end
        end
    end
    ...
    for i = 1:length(fQRS)
        if fQRS(i) <= 40
            tmp = (1:fQRS(i)+40);
        elseif fQRS(i) + 40 >= length(residu_pp)
            tmp = (fQRS(i)-40:length(residu_pp));
        else
            tmp = (fQRS(i)-40:fQRS(i)+40);
        end
        idx_sig = [idx_sig;tmp'];
    end
   ...
   %% SNR Measurement
    ...
    for i = 1:length(fQRS)
        if fQRS(i) <= 40
            tmp = (1:fQRS(i)+40);
        elseif fQRS(i) + 40 >= length(residu)
            tmp = (fQRS(i)-40:length(residu));
        else
            tmp = (fQRS(i)-40:fQRS(i)+40);
        end
        idx_sig = [idx_sig;tmp'];
